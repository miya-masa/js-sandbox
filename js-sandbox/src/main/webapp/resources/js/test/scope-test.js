		...
		function outer() {
			...
			function inner() {
				...
				if (a === 10) {
					...
					ok(typeof c === 'number', "cはスコープの中");
					var c = 30;
					ok(true, "inner関数内 ifブロック内var c定義直後");
					ok(typeof outer === 'function', "outer()はスコープの中");
					ok(typeof inner === 'function', "inner()はスコープの中");
					ok(typeof a === 'number', "aはスコープの中");
					ok(typeof b === 'number', "bはスコープの中");
